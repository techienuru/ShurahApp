...
$query = "
    SELECT a.prod_id, a.prod_name, b.category_name, a.qty_left 
    FROM product a 
    LEFT JOIN category b ON a.category_id = b.category_id 
    WHERE a.qty_left = 0 AND a.date BETWEEN '$start_date' AND '$end_date'";
...
$headers = array('Product ID', 'Product Name', 'Category Name', 'Quantity Left');
$column_widths = array(30, 70, 50, 40); // Adjust these widths as needed
...
while($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell($column_widths[0], 10, $row['prod_id'], 1);
    $pdf->Cell($column_widths[1], 10, $row['prod_name'], 1);
    $pdf->Cell($column_widths[2], 10, $row['category_name'], 1);
    $pdf->Cell($column_widths[3], 10, $row['qty_left'], 1, 1, 'C'); // Center text
}
