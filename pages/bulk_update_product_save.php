...
// query
        $prod_query = "SELECT * FROM update_product WHERE update_product_id='{$bulk_update_id}'";
        ...
        // Insert query
        $query = "INSERT INTO update_product (update_product_id, description, date, user_id) VALUES ('{$bulk_update_id}','{$description}','{$date}','{$user_iid}')";
        $result = mysqli_query($db_connect, $query);

        // Ledger insertion
        
        if ($result) {
            header("location:dashboard.php?page=pages/bulk_update_product_preview&update_id=$bulk_update_id");
        } else {
