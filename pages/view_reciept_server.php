...
$columns = array(
    array( 'db' => 'date', 'dt' => 0 ),
    array( 'db' => 'date', 'dt' => 1 ),
    array( 'db' => 'prod_id',  'dt' => 2 ),
    array( 'db' => 'invoice',  'dt' => 3 ),    
    array( 
        'db' => 'cash',  
        'dt' => 4,
    ...
    array( 
        ...
        'formatter' => function($d, $row){           
            $reprintlink = '<a class="btn btn-xs btn-primary m-r-1em" style="font-size: 12px;" style="margin-top: 1px;" href="dashboard.php?page=pages/reprint&invoice='.$row['invoice'].'&prod_id='.$row['prod_id'].'&cash_tendered='.$row['cash'].'&bal='.$row['balance'].'">Reprint</a>';
            return  ''. $reprintlink . '';
        }
    ), 
);
