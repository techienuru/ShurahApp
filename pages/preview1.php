...
<script language="javascript">
	...
	{ 
		...
		docprint.document.write('</head><body onLoad="self.print()" style="width: 100%; font-size: 14px; font-family: arial;">');     
		...
		<div style="width: 100%;">
			<div>
				<div class="main_logo" style="display: flex;">
					...
					<tbody>
						<tr>
							<td style="font-size: 12px; text-align: left" colspan='2'>Client Name: <strong><?php echo $client ;?></strong></td>
				...
				<div style="display: flex; justify-content: center;">
					<h2 style="border-bottom: 1px solid;"><strong>Sales Receipt</strong></h2>
                </div>
				</div>
			</div>
...
$sourceQuery = "SELECT * FROM sales_order WHERE invoice='{$_GET['invoice']}'";
$sourceResult = mysqli_query($db_connect, $sourceQuery);
// Process and populate data into destination table
	if ($sourceResult) {
		while ($row = mysqli_fetch_assoc($sourceResult)) {
			...
			$movement_type = 'product sales';
			$user_id = $_SESSION['user_id'];
			$qty_in = '0';
			$destQuery = "INSERT INTO product_movement_log (branch_id, prod_code, qty_in, qty_out, movement_type, user_id, movement_date) VALUES ('" . $row['branch_id'] . "', '" . $row['prod_code'] . "', '" . $qty_in . "', '" . $row['qty'] . "', '" . $movement_type . "', '" . $user_id . "', '" . $row['date'] . "')";
			mysqli_query($db_connect, $destQuery);
		}
