...
//echo "Client_ID: ".$client_id;
    $userist_sql = "SELECT SUM(debit) AS total_debit, SUM(credit) AS total_credit, (SUM(debit) - SUM(credit)) AS total_balance FROM client_ledger WHERE client_id = {$client_id}";
        ...
        <div class="row">
            <div class="col-12">
                ...
                <table class="table table striped table-bordered">
                    ...
                    <?php
    $client_id = $_GET['client_id'];
    $query=mysqli_query($db_connect, "SELECT s.date, s.client_id, s.invoice, s.description, s.debit, s.credit, @b := @b + s.credit - s.debit AS balance FROM(SELECT @b := 0.0) AS dummy CROSS JOIN client_ledger AS s WHERE s.client_id ={$client_id} ORDER BY ref_id ASC");      
            ...
            <div>
                <div class="col-5">
                    <div class="form-group">
                        ...
                        <!--<a class="btn btn-info" style="width: 100px;" href="dashboard.php?page=pages/transaction_invoice_details&client_id=<?php echo $_GET['client_id']; ?>&debit=<?php echo $_GET['debit']; ?>&credit=<?php echo $_GET['credit']; ?>&balance=<?php echo $_GET['balance']; ?>&name=<?php echo $_GET['name']; ?>">Details</a>-->
                     </div>
                </div> 
            </div>
    </div>
